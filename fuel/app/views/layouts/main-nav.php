<?php
    $menu = [];
    $active = ( Uri::segment(1) == null ) ? "accueil" : Uri::segment(2);

    $menu["commercial"]["accueil"] = array( "Accueil", 23, 20, Uri::base() );
...
<nav id="main-nav" class="clearfix">
    <ul>
        <?php foreach( $menu["commercial"] as $menu_item_name => $menu_item ) : ?>
            <li <?= ( $menu_item_name == $active ) ? 'class="active"' : '' ?>>
                <a href="<?= isset( $menu_item[3] ) ? $menu_item[3] : Uri::create( Session::get( 'profile' ) . '/' . $menu_item_name ) ?>">
                    <?= Asset::img( "menus/dc-{$menu_item_name}.png",
                            array(
                                "alt" => "{$menu_item[0]}", 
                                "width" => $menu_item[1], 
                                "height" => $menu_item[2]
    ...
    <div class="menu-buttons">
        ...
        <a href="<?= Uri::create( "home/logout" ) ?>" class="deconnexion-button hide-text">Déconnexion</a>
    </div>
</nav>
<!-- End main nav -->